...
public:
	...
	INLINE Quaternion(const Vector3& axis, const Scalar& angle) { m_vec = XMQuaternionRotationAxis(axis, static_cast<float>(angle)); }
	...
	INLINE Quaternion operator~() const { return Quaternion(XMQuaternionConjugate(m_vec)); }
	INLINE Quaternion operator-() const { return Quaternion(XMVectorNegate(m_vec)); }
